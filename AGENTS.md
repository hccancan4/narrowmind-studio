...
Every default config and every preset shipped in the codebase **must** be feasible on that profile. If a feature requires more, gate it behind a hardware-profile check and provide a graceful message. Hardware-profile-keyed training presets live in `crates/orchestrator/src/training/preset.rs`; per-model VRAM guidance in `crates/orchestrator/src/models.rs`.

---

## Tech Stack (Authoritative)

- **Desktop shell**: Tauri v2, React, TypeScript (strict), Vite, xterm.js, Recharts, TanStack Virtual; styling is plain CSS (no Tailwind, no shadcn/ui)
...
- **ML libraries**: Unsloth (default training), Axolotl (alt), llama.cpp / llama-cpp-python (inference + export), LanceDB + BM25/RRF hybrid retrieval (RAG), sentence-transformers (BGE-small embeddings), datasets / PyMuPDF / trafilatura / wikipedia-api / EbookLib (ingestion); eval is the in-app LLM-judge pipeline (`run_eval`, `crates/orchestrator/src/tools/eval.rs`), not lm-evaluation-harness
...
- IPC types are hand-maintained typed wrappers in `apps/desktop/src/lib/tauri.ts`; keep names + arg shapes in sync with the `invoke_handler!` macro in `apps/desktop/src-tauri/src/lib.rs`. No binding generator (tauri-specta was never adopted).
...
1. **Test as you build.** Each phase adds unit tests to the real suites: `cargo test --workspace` and `uv run pytest`. TS tests land with their phases (the `pnpm -r test` scripts are currently echo stubs). Acceptance criteria in `ROADMAP.md` should map to integration tests where feasible.
